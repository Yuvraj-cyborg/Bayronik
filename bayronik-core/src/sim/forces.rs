...
pub fn interpolate_forces_to_particles(
    ...
    for p in &mut particles.particles {
        ...
        let indices = [
            (((i % n_isize + n_isize) % n_isize * n_isize + (j % n_isize + n_isize) % n_isize)
                * n_isize
                + (k % n_isize + n_isize) % n_isize) as usize,
            ((((i + 1) % n_isize + n_isize) % n_isize * n_isize
                + (j % n_isize + n_isize) % n_isize)
                * n_isize
                + (k % n_isize + n_isize) % n_isize) as usize,
            (((i % n_isize + n_isize) % n_isize * n_isize
                + ((j + 1) % n_isize + n_isize) % n_isize)
                * n_isize
                + (k % n_isize + n_isize) % n_isize) as usize,
            (((i % n_isize + n_isize) % n_isize * n_isize + (j % n_isize + n_isize) % n_isize)
                * n_isize
                + ((k + 1) % n_isize + n_isize) % n_isize) as usize,
            ((((i + 1) % n_isize + n_isize) % n_isize * n_isize
                + ((j + 1) % n_isize + n_isize) % n_isize)
                * n_isize
                + (k % n_isize + n_isize) % n_isize) as usize,
