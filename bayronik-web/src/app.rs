...
impl BayronikApp {
    ...
    fn tab_camels(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        if self.camels_info.is_none() {
            ...
            ui.label("Make sure the FastAPI server is running: make server");
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_enabled_ui(!running, |ui| {
                if ui.button(egui::RichText::new("🚀 Run N-Body + Emulator").strong().size(16.0)).clicked() {
                    ...
                    let map = bayronik_core::run_simulation(
                        self.nbody_seed,
                        self.nbody_grid_res,
                        self.nbody_box_size,
                        0.01,
    ...
    fn tab_about(&self, ui: &mut egui::Ui) {
        ...
        egui::Grid::new("about_grid").striped(true).show(ui, |ui| {
            ui.label("bayronik-core");
            ui.label("Rust Particle-Mesh N-body simulator (runs in-browser via WASM)");
            ui.end_row();
            ui.label("bayronik-model");
            ui.label("Python U-FNO training pipeline + FastAPI inference server");
            ui.end_row();
            ui.label("bayronik-web");
            ui.label("This app — Rust/egui/WASM frontend");
            ui.end_row();
            ui.label("bayronik-infer");
            ui.label("Rust TUI for terminal-based inference (ratatui + libtorch)");
            ui.end_row();
        });
