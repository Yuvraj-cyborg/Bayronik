...
use crate::visualization::{Colormap, array_to_colorimage};
...
impl BayronikApp {
    ...
    fn update_textures(&mut self, ctx: &egui::Context) {
        ...
        if let Some(input) = &self.input_map {
            ...
            self.input_texture =
                Some(ctx.load_texture("input_map", img, TextureOptions::default()));
        }

        if let Some(output) = &self.output_map {
            let img = array_to_colorimage(output, n, n, &self.colormap);
            self.output_texture =
                Some(ctx.load_texture("output_map", img, TextureOptions::default()));
            ...
            if let Some(input) = &self.input_map {
                ...
                self.diff_texture =
                    Some(ctx.load_texture("diff_map", diff_img, TextureOptions::default()));
            }
        }
    }
}
